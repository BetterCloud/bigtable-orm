...
public class GeneratedKeyBuilderTest {
  ...
  public void testConstantKeyEntityKeyBuilderContainsNoCustomizationSteps() {
    ...
    assertEquals(
        KeyBuilderTableConfiguration.ConstantKeyEntity.CONSTANT_KEY_COMPONENT, key.toString());
    assertArrayEquals(
        Bytes.toBytes(KeyBuilderTableConfiguration.ConstantKeyEntity.CONSTANT_KEY_COMPONENT),
        key.toBytes());
  }

  @Test(expected = NullPointerException.class)
  public void testSingleKeyComponentEntityKeyBuilderWithNullValueThrowsNullPointerException() {
    SingleKeyComponentEntity.keyBuilder().myValue(null).build();
  }
  ...
  public void testSingleKeyComponentEntityKeyBuilderBuildsKeyContainingDefinedValue() {
    ...
    final Key<SingleKeyComponentEntity> key =
        SingleKeyComponentEntity.keyBuilder().myValue(value).build();
  ...
  public void testMultiKeyComponentEntityKeyBuilderBuildsKeyJoiningAllPartsWithDefaultDelimiter() {
    ...
    final Key<MultiKeyComponentEntity> key =
        MultiKeyComponentEntity.keyBuilder().first(first).second(second).build();
  ...
  public void testCustomKeyDelimiterEntityKeyBuilderBuildsKeyJoiningAllPartsWithCustomDelimiter() {
    ...
    final Key<CustomKeyDelimiterEntity> key =
        CustomKeyDelimiterEntity.keyBuilder().first(first).second(second).build();

    assertNotNull(key);

    final String expected =
        String.join(
            KeyBuilderTableConfiguration.CustomKeyDelimiterEntity.CUSTOM_DELIMITER, first, second);
  ...
  public void testMultiKeyComponentEntityWithConstantKeyBuilderBuildsKeyJoiningAllParts() {
    ...
    final Key<MultiKeyComponentEntityWithConstant> key =
        MultiKeyComponentEntityWithConstant.keyBuilder().first(first).third(third).build();
    ...
    final String expected =
        String.join(
            "::",
            first,
            KeyBuilderTableConfiguration.MultiKeyComponentEntityWithConstant.CONSTANT_KEY_COMPONENT,
            third);
  ...
  public void testCustomKeyComponentTypeEntityKeyBuilderBuildsKeyFromCustomType() {
    ...
    final Key<CustomKeyComponentTypeEntity> key =
        CustomKeyComponentTypeEntity.keyBuilder().custom(custom).build();
  ...
  private class KeyBuilderTableConfiguration {
    ...
    @Entity(keyComponents = {@KeyComponent(constant = ConstantKeyEntity.CONSTANT_KEY_COMPONENT)})
    ...
    @Entity(keyComponents = {@KeyComponent(name = "myValue")})
    ...
    @Entity(keyComponents = {@KeyComponent(name = "first"), @KeyComponent(name = "second")})
    ...
    @Entity(
        keyDelimiter = CustomKeyDelimiterEntity.CUSTOM_DELIMITER,
        keyComponents = {@KeyComponent(name = "first"), @KeyComponent(name = "second")})
    ...
    @Entity(keyComponents = {@KeyComponent(name = "custom", type = UUID.class)})
