...
abstract class AbstractDao<T extends Entity> {
    ...
    return keys.stream()
        ...
        .map(
            key -> {
              ...
              for (final Column column : columns) {
                get.addColumn(
                    Bytes.toBytes(column.getFamily()), Bytes.toBytes(column.getQualifier()));
              }
              return get;
            })
  ...
  <K extends Key<T>> Scan keysToScan(
      final K startKey,
  ...
  <K extends Key<T>> PutResultDto<K, T> entitiesToPuts(final Map<K, T> entities)
      throws IOException {
      ...
      for (final Column column : columns) {
        ...
        if (column.isVersioned()) {
          final long timestamp =
              Optional.ofNullable(sourceDelegate.getColumnTimestamp(column))
                  .orElseGet(() -> Instant.now().toEpochMilli());

          put.addColumn(
              Bytes.toBytes(column.getFamily()),
          ...
          put.addColumn(
              Bytes.toBytes(column.getFamily()), Bytes.toBytes(column.getQualifier()), bytes);
        }
      }
      ...
      return putTuples.stream()
          .collect(
              Collectors.collectingAndThen(
                  Collectors.toMap(PutTuple::getKey, PutTuple::getResult),
